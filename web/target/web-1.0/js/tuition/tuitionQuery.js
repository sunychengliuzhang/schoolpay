$(function(){
	...
	$('#id_seciton-city',maindom).delegate('.select-elem-detail','click',function(e){
		...
		if(!that.hasClass('select')){
			...
			provincesId = $("#province_id").val();
			//切换地市
			loadChargeCorps(provincesId,cityId);
		}
	});
...
//加载市的值
function loadCitysData(provinceId){
	var city = "";
	$.ajax({
        ...
        success: function (data){
        	...
        	if(data.success == "success"){
        		...
        		cityId = $("#city_id").val();
            	//加载缴费单位
            	loadChargeCorps(provinceId,cityId);
        	}else{
...
//加载缴费单位
function loadChargeCorps(provinceId,cityId){
	$.ajax({
