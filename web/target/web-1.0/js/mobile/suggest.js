...
//		}	
		...
		function processKey(e) {				
			...
			function displayItems(items) {
				var html = '';
				if (items=='') {//热门城市遍历
					alert(options.hot_list);
					for(h in options.hot_list){
						html += '<div class="city-wrap"  onclick="returnCityToBack("${item.real_value}","${item.desc_value}")"><i class="cur-icon"></i><span class="city-con">${item.desc_value}</span></div>';
					}
	...
	$.fn.suggest = function(source, options) {	
		if (!source)
			return;		
		//citys,{hot_list:commoncitys,attachObject:"#suggest2"}
		options = options || {};
